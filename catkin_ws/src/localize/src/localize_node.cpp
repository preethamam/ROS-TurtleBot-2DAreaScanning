...
void set_goal_slam(double &curr_x, double &curr_y, double &curr_z_angle, bool &finished, Direction &d, bool &is_reverse, bool &coverage, int green_detect) {
	...
	static double length = 3; // forward length in meters
	static double width = 0.5;
	static double stride = 0.2;

	static int state = 0;
	static bool done = false;
	is_reverse = false;
	const double rotation = 0.000092;
...
// 		}
	...
	if(d == ROTATE) {
		if(abs(goal_z_angle - z_angle) <= 0.000006) {
			reached = true;
	...
	)
{
	...
	if (coverage == false){ 
		...
		if (z_angle >10000)
			z_angle = z_angle_prev;
		
		z_angle_prev = z_angle;
