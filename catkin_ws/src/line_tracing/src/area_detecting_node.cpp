...
#include <line_tracing/cmd_stamped.h>
...
// green
  int lowH = 35, lowS = 50, lowV = 150;
  int highH = 90, highS = 255, highV = 255;
...
public:
    ...
    : it_(nh_)
  {
    ...
    theta_pub_ = nh_.advertise<line_tracing::theta_stamped>("/detect_msg", 1);
    //cmd_pub_ = nh_.advertise<line_tracing::cmd_stamped>("/direction_cmd", 1); 
  }
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    static int seq_theta = 0;
    // static int seq_cmd = 0;
    ...
    try
    {
      ...
      int largest_index = 0;
      cout << "before largest_index"<<endl;
      line_tracing::theta_stamped msg;

      if (contours.size() == 0){
        //publish detection
            std::cout<<"no green" <<std::endl;
            msg.header.seq = seq_theta++;
            msg.header.stamp = ros::Time::now();
            msg.header.frame_id = "/detect_msg";
            //msg.data = theta;
            msg.detect = 0;
            std::cout<<"detect :0"<<std::endl;
        ...
        if (n > 300){
          ...
          for (int i=0; i<contours[largest_index].size(); i++){
            if (contours[largest_index][i].x < (rows/2)){
